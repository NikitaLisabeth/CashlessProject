...
namespace nmct.ba.cashlessproject.api.Helper
{
    public class DAKassaManagement
    {
        ...
        public static int UpdateAccount(RegistersKlant kassa, IEnumerable<Claim> claims)
        {
            ...
            DbParameter par1 = Database.AddParameter(CreateConnectionString(claims), "@RegisterName", kassaName);
            DbParameter par2 = Database.AddParameter(CreateConnectionString(claims), "@Device", device);
            ...
            DbParameter par11 = Database.AddParameter(CreateConnectionString(claims), "@RegisterId", id);
            DbParameter par12 = Database.AddParameter(CreateConnectionString(claims), "@EmployeeId", medewerkerid);
            DbParameter par13 = Database.AddParameter(CreateConnectionString(claims), "@FromDate", from);
            DbParameter par14 = Database.AddParameter(CreateConnectionString(claims), "@UntilDate", until);
